...
async def test_platform_work_worker_suppresses_stale_scoreable_with_scoring_error_result_pending_submission() -> None:
    ...
    result = result.model_copy(
        update={
            "result": result.result.model_copy(update={"run": result.result.run.model_copy(update={"response": None})})
        }
    )
    score_calls = 0
...
async def test_worker_groups_assignments_by_artifact_and_reports_all_active_attempts() -> None:
    ...
    assert set(worker._active_artifacts) == {(batch_id, artifact.artifact_id) for artifact in artifacts}
    ...
    for (_batch_id, artifact_id), group in worker._active_artifacts.items():
        ...
        assert all(
            record.assignment.artifact.artifact_id == artifact_id for record in group.assignment_records.values()
        )
...
async def test_worker_expires_queued_reservation_inside_started_group_without_dropping_reassignment() -> None:
    ...
    assert group.local_inflight_count() == 1
    assert {record.assignment.task.task_id for record in group.assignment_records.values()} == {
...
async def test_worker_defers_delivery_failure_until_started_sibling_result_is_collected() -> None:
    ...
    assert {(identity.task_id, identity.attempt_number) for identity in worker._active_attempts()} == {
...
async def test_worker_keeps_same_artifact_assignments_separate_across_batches() -> None:
    """Future failure: a repeated artifact ID must not merge work from different batches."""
...
async def test_worker_collects_all_results_before_clearing_closed_artifact_group() -> None:
    ...
    artifact = _artifact(uid=1)
    assignments = tuple(
        _assignment(batch_id=batch_id, artifact=artifact, task=_task(f"task-{index}")) for index in range(2)
    )
    submitted: list[tuple[PlatformOwnedTaskResult, ...]] = []
    ...
    assert all(
        result.terminal_attempt.terminal_effect is MinerTaskAttemptTerminalEffect.TASK_RESULT for result in submitted[0]
    )
    assert worker._active_artifacts == {}
    assert worker._results_pending_submission == []
