...
Champion emission remains active and additive. Data versions 1 through 8 retain the historical 20% champion maximum. From the first successful version-9 batch, an artifact's champion maximum follows 50% on selection, 25% on its first consecutive successful retention, and a 12.5% floor afterward; its score scales that maximum. Failed batches do not advance or break the sequence. Participant emission uses the latest terminal source batch with finalized tasks and artifacts. A failed terminal batch divides the entire post-champion remainder equally among distinct participant hotkeys. A successful data-version-7-or-later batch divides that remainder per reward-eligible artifact using `participation stage × novelty`: top 50% `1`, top 10% `2`, main `5`; near duplicate `1`, notable change `3`; novel `5` in versions 7/8 and `10` from version 9. The highest distribution weight is `25` historically and `50` from version 9. Shares are aggregated to hotkeys after artifact allocation. Champion emission is never novelty-multiplied. Data versions 1 through 6 retain their historical reward calculation. Registered participant hotkeys receive weight at their current metagraph UID; unregistered shares burn through owner `uid=0`.
