...
Similarity judging uses `SIMILARITY_LLM_*` for provider, temperature, timeout, output tokens, and retry policy. The default primary model remains `google/gemma-4-31B-turbo-TEE`, followed by `moonshotai/Kimi-K2.5-TEE` and then `zai-org/GLM-5-TEE` if the current candidate exhausts provider retries. The platform selects the structurally closest eligible artifact, which can be a recent champion or an earlier same-batch challenger, then sends that reference script plus the candidate diff to validators. Validators return `duplicate`, `near_duplicate`, or `novel` with structured reasoning. Deriving from reference code is encouraged: `duplicate` means there is no concrete behavior change; `near_duplicate` means changed behavior inside a substantially shared primary pipeline; and `novel` means the candidate reorganizes or replaces the primary ordinary-case control or data flow relative to that reference. This pairwise result does not claim first-seen or corpus-wide uniqueness. If structural selection returns `no_eligible_reference`, Platform assigns `novel` automatically without requesting a validator judgment; that result carries no pairwise architectural evidence. Both changed-behavior categories enter task scoring. A `novel` classification receives remaining-emission distribution weight after the miner reaches the main round, top 10% or top 50%.
...
Champion emission remains active. Participant emission uses the latest terminal source batch with finalized tasks and artifacts. For a failed terminal batch, champion emission is reserved first and the entire remainder is divided equally among distinct participant miner hotkeys, one share per hotkey. Successful `near_duplicate` and `novel` miners receive the same fixed assigned reward: `0.004` in the top 10% and `0.002` through the top 50%; duplicate-filtered and zero-response artifacts remain visible with zero reward and `no_reward`, and each distinct main participant retains an additional `0.008`. After champion and participant assigned rewards, `novel` miners divide the entire remaining emission with exclusive main-round `5`, otherwise top-10% `3`, otherwise top-50% `1` weight. Score `0` still gets `0` unless the novel miner reached the main round. Registered participant hotkeys receive weight at their current metagraph UID, in addition to any champion emission for the same UID. Shares for unregistered hotkeys burn through owner `uid=0`. `champion_uid` reports the active champion emission component's current UID when champion emission is active.

Monitoring reports these allocations as hotkey totals with expandable artifact attribution for assigned, main-round, novelty and failed-batch rewards. A failed-batch share is attributed to the latest challenger artifact for that hotkey, or to its incumbent only when no challenger exists. This reporting structure does not change the hotkey-based weight calculation and adds no database state.
