...
Validator scoring keeps `SCORING_LLM_PROVIDER` configurable, but the scoring model contract is fixed in code to `openai/gpt-oss-120b-TEE` with `reasoning_effort="high"`. Do not set `SCORING_LLM_MODEL` or `SCORING_LLM_REASONING_EFFORT` in `.env`.
