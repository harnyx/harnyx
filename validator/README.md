...
Champion emission remains active and additive. Participant emission uses the latest terminal source batch with finalized tasks and artifacts. A failed terminal batch divides the entire post-champion remainder equally among distinct participant hotkeys. A successful data-version-7 batch divides that remainder per reward-eligible artifact using `participation stage × novelty`: top 50% `1`, top 10% `2`, main `5`; near duplicate `1`, notable change `3`, novel `5`. The highest distribution weight is therefore `25`. Shares are aggregated to hotkeys after artifact allocation. Champion emission is never novelty-multiplied. Data versions 1 through 6 retain their historical reward calculation. Registered participant hotkeys receive weight at their current metagraph UID; unregistered shares burn through owner `uid=0`.
