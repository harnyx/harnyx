...
`RATING_COMPETITION_ENABLED` defaults to `false`. Enable it only for a coordinated rating test with Platform and with the normal Platform client and scoring-provider configuration available. Platform must select your exact registered source revision as compatible; enabling the local worker alone does not make the validator eligible. This lane does not change white-box work or activate black-box emission.
