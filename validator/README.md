...
Champion emission remains active. Participant emission uses the latest terminal source batch with artifacts. Failed terminal batches add fixed `0.004` compensation for each distinct participant miner hotkey. Successful terminal batches use a `0.008` base and tier participant emission by normalized artifact score: the top 10% earns a `2x` tier maximum, participants through the top 50% earn a `1x` tier maximum, and lower-ranked participants get `0`. Within an earned tier, `near_duplicate` receives 25% and `novel` receives 100%, so their top-tier allocations are `0.004` and `0.016` and their middle-tier allocations are `0.002` and `0.008`. Each distinct main participant receives an additional `0.008`. Score `0` still gets `0`. Completed terminal batches without successful scoring evidence burn participant emission instead of using historical monitoring fallback rows. Registered participant hotkeys receive weight at their current metagraph UID, in addition to any champion emission for the same UID. The final owner `uid=0` remainder, including shares for participant hotkeys that are not currently registered, burns miner emission and is not paid to the owner. `champion_uid` reports the active champion emission component's current UID when champion emission is active.
