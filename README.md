...
`GET /v1/weights` still uses latest champion weights for champion emission and the latest terminal source batch with finalized tasks and artifacts for participant emission. For a failed terminal batch, champion emission is reserved first and the entire remainder is divided equally among distinct participant hotkeys, one share per hotkey. Successful `near_duplicate` and `novel` miners receive the same fixed assigned reward: `0.004` in the top 10% and `0.002` through the top 50%; duplicate-filtered and zero-response artifacts remain visible with zero reward and `no_reward`, and each distinct main participant hotkey retains an additional `0.008`. After champion and participant assigned rewards, selected `novel` artifacts divide the entire remaining emission with exclusive main-round `5`, otherwise top-10% `3`, otherwise top-50% `1` weight. Weight `5` requires that the selected artifact itself reached main; it cannot inherit main participation from another artifact under the same hotkey. Score `0` still gets `0` unless that selected novel artifact reached the main round. The participant allocation is projected to the current metagraph UID when registered; shares for unregistered hotkeys burn through owner `uid=0`.
