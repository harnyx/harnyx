...
- Miner emission keeps champion emission active and adds participant emission from the latest terminal source batch with artifacts. Successful batches use score tiers plus the artifact's novelty classification; failed batches use fixed `0.004` compensation. The exact allocations are described below. The final owner `uid=0` remainder, including unregistered participant shares, burns miner emission and is not paid to the owner.
